...
static const char* HMAC_SECRET = "I_AM_AETHORIAN";
...
static void scan_processes(){
  ...
  do{
    /* UNICODE-safe conversion of pe.szExeFile to UTF-8/narrow */
    char nbuf[MAX_PATH];
  #ifdef UNICODE
    WideCharToMultiByte(CP_UTF8, 0, pe.szExeFile, -1, nbuf, MAX_PATH, NULL, NULL);
  #else
    strncpy(nbuf, pe.szExeFile, MAX_PATH-1); nbuf[MAX_PATH-1] = '\0';
  #endif
    strlower(nbuf);
